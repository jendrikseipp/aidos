...
void compute_h2_mutexes(const vector <Variable *> &variables,
	...
	if((!regression && update_progression) ||
	   (regression && update_regression)){
	    ...
	    if(regression && disable_bw_h2) continue;
	    ...
	    if(regression) total_mutexes_bw += mutexes_detected;
	    else total_mutexes_fw += mutexes_detected;
...
bool H2Mutexes::initialize(const vector <Variable *> &variables,
	...
	for (size_t j = 0; j < variables[i]->get_range(); j++){
	...
	for(size_t val1 = 0; val1 < num_vals[var]; ++val1){
	    int p_index_1 = p_index[var][val1];
	    for(size_t val2 = val1 + 1; val2 < num_vals[var]; ++val2){
...
void H2Mutexes::init_values_progression(const vector <Variable *> &variables,
