...
void BreadthFirstSearch::reward_progress() {
...
void BreadthFirstSearch::start_f_value_statistics(EvaluationContext &eval_context) {
    if (f_evaluator) {
...
void BreadthFirstSearch::update_f_value_statistics(const SearchNode &node) {
    if (f_evaluator) {
        /*
...
static SearchEngine *_parse(OptionParser &parser) {
    ...
    BreadthFirstSearch *engine = nullptr;
    if (!parser.dry_run()) {
        opts.set<bool>("mpd", false);
        engine = new BreadthFirstSearch(opts);
    }
