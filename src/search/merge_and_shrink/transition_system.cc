...
void TransitionSystem::label_inheritance(const Bitset &irrelevant_labels_in_all_other_ts) {
    if (!irrelevant_labels_in_all_other_ts.count()) {
        return;
    }

    // Undo the grouping of locally equivalent labels and compute the
    // forward graph (indexed by states).
    vector<set<pair<int, int>>> forward_graph(num_states);
    vector<vector<Transition>> new_transitions_by_group_id(transitions_by_group_id.size());
    for (const GroupAndTransitions &gat : *this) {
        const vector<Transition> &transitions = gat.transitions;
        const LabelGroup &label_group = gat.label_group;
        for (int label_no : label_group) {
            for (const Transition &transition : transitions) {
                forward_graph[transition.src].insert(make_pair(transition.target, label_no));
            }
            new_transitions_by_group_id[label_no] = transitions;
        }
    }
    label_equivalence_relation->reset();
    vector<vector<Transition>>().swap(transitions_by_group_id);
    transitions_by_group_id.swap(new_transitions_by_group_id);

    // Inherit transitions of all labels that are irrelevant in all other
    // transition systems.
    int num_labels = irrelevant_labels_in_all_other_ts.size();
    for (int label_no = 0; label_no < num_labels; ++label_no) {
        if (label_equivalence_relation->is_current_label(label_no)
                && irrelevant_labels_in_all_other_ts[label_no]) {
            int group_id = label_equivalence_relation->get_group_id(label_no);
            assert(group_id == label_no);
            const vector<Transition> &transitions = transitions_by_group_id[group_id];

            // fix-point iteration: propagate label inheritance until no more
            // labels are inherited.
            while (true) {
                bool changed = false;
                int num_transitions = transitions.size();
                // Only consider transitions of the current label that existed
                // before the current round of inheritance.
                for (int i = 0; i < num_transitions; ++i) {
                    const Transition &t = transitions[i];
                    int src = t.src;
                    int target = t.target;
                    if (src != target) {
                        // We can ignore self-loops because in this case there
                        // are no transitions to be inherited. This is important
                        // for set_difference to work (requires non-overlapping
                        // sequences).
                        const set<pair<int, int>> &target_transitions = forward_graph[target];
                        set<pair<int, int>> &source_transitions = forward_graph[src];
                        vector<pair<int, int>> inherited_transitions;
                        set_difference(target_transitions.begin(), target_transitions.end(),
                                       source_transitions.begin(), source_transitions.end(),
                                       inserter(inherited_transitions, inherited_transitions.end()));
                        source_transitions.insert(inherited_transitions.begin(),
                                                  inherited_transitions.end());
                        if (!changed && !inherited_transitions.empty()) {
                            changed = true;
                        }
                        for (const pair<int, int> &inherited_trans : inherited_transitions) {
    ...
    vector<set<pair<int, int>>>().swap(forward_graph);
    for (vector<Transition> &transitions : transitions_by_group_id) {
...
void TransitionSystem::prune_transitions_of_goal_states() {
    for (vector<Transition> &transitions : transitions_by_group_id) {
        for (vector<Transition>::iterator it = transitions.begin();
            it != transitions.end(); ) {
