...
configs = [
    ...
    IssueConfig(
        "dfp-modlabelapproxb50k",
        ["--search", "astar(merge_and_shrink(shrink_strategy=shrink_mod_label_approx(max_states=50000,threshold=1,greedy=false),merge_strategy=merge_dfp,label_reduction=exact(before_shrinking=true,before_merging=false),only_dead_end_detection=false))"]),
    IssueConfig(
        "dfp-modlabelapproxb50k-dead",
        ["--search", "astar(merge_and_shrink(shrink_strategy=shrink_mod_label_approx(max_states=50000,threshold=1,greedy=false),merge_strategy=merge_dfp,label_reduction=exact(before_shrinking=true,before_merging=false),only_dead_end_detection=true))"]),
]
